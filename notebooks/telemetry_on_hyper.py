...
# Never do in 5 lines what you can do in 1
# What's an 'optimization'?...
accuracy = [{
    'id': int(row.split(', ')[0].split(' ')[-1]),
    'MD': int(row.split(', ')[1].split(' ')[-1]),
    'MF': float(row.split(', ')[2].split(' ')[-1]),
    'N': int(row.split(', ')[3].split(' ')[1]),
    'accuracy': float(row[row.find('Accuracy') + len('Accuracy') + 2:row.find('%')])
} for row in data]
